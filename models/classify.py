...
class FaceNet64(nn.Module):
    def __init__(self, num_classes = 1000):
        ...
        # 特征提取模块【evolve.IR_50_64，Inception-ResNet】
...
# 加载目标模型和加载预训练模型参数【特征提取部分的参数】
def get_classifier(model_name, mode, n_classes, resume_path):
    # 模型训练模式【reg/vig】
    # reg【Regression，回归】：监督学习
    # vib【Variational Information Bottleneck，变分信息瓶颈】：无监督学习
    if model_name == "VGG16":
        if mode == "reg":
            net = VGG16(n_classes)
        elif mode == "vib":
    ...
    if model_name in ['FaceNet', 'FaceNet_all', 'FaceNet_64', 'IR50', 'IR152']:
        if resume_path is not "":
            print("Resume")
            # torch.load()加载预训练模型参数文件，返回一个参数字典
