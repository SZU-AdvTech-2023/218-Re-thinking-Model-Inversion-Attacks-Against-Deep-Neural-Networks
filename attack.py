...
# 获取p_reg【使用5000张来自公共数据集的图像来估计μpen和σ2pen】
def get_act_reg(train_loader, T, device, Nsample=5000):
    all_fea = []
    with torch.no_grad():
        for batch_idx, data in enumerate(train_loader): # batchsize = 50
            # print(data.shape)
...
def dist_inversion(G, D, T, E, iden, lr=2e-2, momentum=0.9, lamda=100, \
    ...
    # ./attack_results/kedmi_300ids/celeba_IR152/latent/0_0_iter_0_2400_dis.npy
    outputs_z = "{}_iter_{}_{}_dis.npy".format(prefix, random_seed, iter_times-1)

    # 不存在该文件才开始攻击
    if not os.path.exists(outputs_z):
        # ./attack_results/kedmi_300ids/celeba_IR152/latent/0_0_iter_0_0_dis
        outputs_z = "{}_iter_{}_{}_dis".format(prefix, random_seed, 0)
        # ./attack_results/kedmi_300ids/celeba_IR152/latent/0_0_iter_0_0_label
        ...
        for i in range(iter_times):
            ...
            # 每300次保存潜在分布【均值和标准差】、攻击身份【60个】、评估优化效果
            if (i + 1) % 300 == 0:
                ...
                with torch.no_grad():
                    ...
                    # ./attack_results/kedmi_300ids/celeba_IR152/imgs_ours/0_300.png
